    ...
    def split(self, bytes):
        ...
        while len(bytes) > 0:
            ...
            if self.config.truncate_activated:
                ...
                if not patternStackTrace.search(decoded_msg) and len(decoded_msg) > self.config.truncate_max_msg_length:
                    decoded_msg = '{} __TRUNCATED__ {}'.format(decoded_msg[:self.config.truncate_max_msg_length//2], decoded_msg[-self.config.truncate_max_msg_length//2:])
